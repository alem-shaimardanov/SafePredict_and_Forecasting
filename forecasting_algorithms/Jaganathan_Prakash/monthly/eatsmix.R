...
forx = function(x, h.fc, frequency = freq, transform = TRUE) {
  ...
  ets.bc    = forecast::forecast(ets(x.bc), h = h.fc, lambda = lambda,biasadj=F)$mean
  arima.bc  = forecast::forecast(auto.arima(x.bc), h = h.fc, lambda = lambda,biasadj=F)$mean
  etsd.bc   = forecast::forecast(ets(x.bc, damped = TRUE), h=h.fc, lambda = lambda,biasadj=F)$mean
...
save(for.m.eats, file = paste0("m_eats_MONTHLY_m4_final.rda"))
