...
mydata.forpro <- mclapply(mydata.list,function(x) ts(as.vector(t(x)),frequency = freq,end=c(2018,12)),mc.cores = ncore)
...
write.csv(fpro.m,file="m4_quarter_fpro_t_final.csv",row.names=FALSE)
