...
MyData <- read.csv(file="Yearly-train.csv", header=TRUE, sep=",")
...
forpro.1 <- read.csv(file="!Project01 Numeric Output final yearly.csv", header=TRUE, sep=",",strip.white=TRUE)
...
forepro.list <- setNames(split(forpro.2[,c(-1:-5,-12)],seq(nrow(forpro.2))),forpro.2[,2])
